...
- `src/api/services/mods/platform-modrinth.service.ts` — Modrinth got 客戶端（免 key）：同四方法。含 `mapMrDep`（required/optional 保留）、`primaryFile`（primary 缺則第一個）、`MR_LOADER` facet 對映。
...
- `src/api/controllers/server-modpack.controller.ts` — 新增 `addFileFromPlatform`；imports 加兩平台 service 與模組級實例。
...
1. **進池完全複用既有管線**：`addFileFromPlatform` 取得平台檔案資訊後，CF 走 `ensureCurseforgeFileInPool`、Modrinth 走 `ensureModrinthFileInPool`（皆 `utils/modpool/pool`）。cf_file_hashes 快取、headObject 免下載、x-api-key、全域併發 16、重試退避、**順路 metadata 解析**均由 pool 內建，controller 不複製亦不改該層（守界 §7）。
...
### 偏離記錄（既有慣例不一致，本任務不動既有函式，留待後續 F15 對齊）
- **(a) CF key 缺席狀態碼**：新端點回 **503**（規格 §3.5），既有 `importVersion`（`server-modpack.controller.ts:137`）回 **400**。已與用戶確認：新端點依規格 503，既有 400 守界不動。`src` 先前無 503 前例，本任務首用（`AppError`/inline `res.status(503)`）。
- **(b) published guard 狀態碼**：新 `addFileFromPlatform` 回 **409**（比照 `restoreFile`/`updateFilePolicy`），既有 `addFile` 回 **400**。本任務不動 `addFile`。
...
```
- 新增 4 檔 **23 測全綠**（既有 96 → 119）。既有 `tests/modpool/` 與 `tests/mods/` 未破壞。
...
**測試覆蓋映射說明（誠實標註）**：任務包 §5-4 列的「Modrinth headObject 命中免下載」「cf_file_hashes 命中零下載」屬 **pool 層內部行為**，from-platform 完全委派該層，故於 from-platform 測試中**以 mock 攔截 pool 呼叫並斷言委派 + 正確入參**，而該兩項快取/headObject 零下載路徑本身由既有 `tests/modpool/pool.curseforge.test.ts`（cf 快取命中不下載）與既有 modpool 套件覆蓋，本任務未重測 pool 內部以免重複。
...
本環境無真實 CurseForge API key、無 dev DB 連線、無法對真實 CF/Modrinth 平台發網路請求，故 §6 真機流**誠實標為待人工**，尚未執行。待人工於具 CF key 的 dev 環境執行並回填：
1. 搜尋（CF + Modrinth 各一）→ 版本清單（含依賴欄）。
2. from-platform 加入 dev 某 draft，一次命中快取路徑（cf_file_hashes / Modrinth headObject）、一次全新下載路徑，共 ≥2 檔。
3. 核對池物件 `mods/files/{sha256}{ext}` 存在、draft entry 正確、`mod_metadata` 有 row（含 deps）。

## 回歸守門

- 既有 17 檔測試全綠，`tests/modpool/`、`tests/mods/`、`tests/routes/` 契約未破壞。
- 瀏覽端點為淨新增路由；from-platform 為新增 handler，不改 `addFile`/`importVersion`/`publishVersion` 等既有函式；`mods.service.ts` 未動。
- 進池/refs/metadata 均複用既有 `utils/modpool/pool` 路徑，無複製重寫管線。
- 無新增 HTTP client 依賴（沿用 `got` v11，CJS 安全）；`yarn build` 綠佐證未破壞 CJS 建置。

## 守界聲明

- 僅實作 F13a-3 任務包範圍 + 兩處已確認決策（CF key 缺席 503；published guard 409）。
- 不動前端/Tauri（F13a-4）、不做整包匯入（F15）、不做 mod_id 反向解析、不加 proxy 快取層、不改 `mods.service.ts`、不動 `importVersion` 的 400。
- 進池/metadata 未小幅重構即可複用，無需回報改既有管線。
- 未動 Nymless / allay_core。
- 未自行裁決設計疑義：CF key 狀態碼與 service 佈局皆先以 AskUserQuestion 取得確認後才落地；三點補充（downloadUrl null 佐證、偏離雙記、全域併發 16 確認）已納實作與本報告。
